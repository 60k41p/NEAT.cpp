...
/* File: Serialization.h Description: Stream helpers for genome/trait persistence: stream validation, trait value/map read-write with variant-index markers and
 * quoted strings, and round-trip floating-point precision. */
...
namespace NEAT {
    namespace Serialization {
        ...
        inline void RequireStream(const std::ios &stream, const char *context) {
            if (!stream) throw std::runtime_error(std::string(context) + ": malformed serialized data.");
        }
        ...
        inline void WriteTraitValue(std::ostream &output, const TraitType &value) {
            ...
            std::visit(
                [&output](const auto &item) {
                    ...
                    else if constexpr (std::is_same_v<T, intsetelement> || std::is_same_v<T, floatsetelement>)
                        output << item.value;
                    else
                        output << item;
                },
        ...
        inline void WriteTraitValues(std::ostream &output, const std::vector<TraitType> &values) {
            output << values.size();
        ...
        inline std::vector<TraitType> ReadTraitValues(std::istream &input) {
            ...
            for (std::size_t i = 0; i < count; ++i) values.push_back(ReadTraitValue(input));
            return values;
        }

        inline void WriteTraits(std::ostream &output, const std::string &marker, const std::map<std::string, Trait> &traits) {
        ...
        inline std::map<std::string, Trait> ReadTraits(std::istream &input) {
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                if (token != "Trait") throw std::runtime_error("trait map: missing Trait marker.");
        ...
        inline void WriteTraitParameters(std::ostream &output, const std::string &marker, const std::map<std::string, TraitParameters> &schemas) {
            output << marker << ' ' << schemas.size() << '\n';
            for (const auto &[key, schema] : schemas) {
                output << "TraitSchema " << std::quoted(key) << ' ' << schema.m_ImportanceCoeff << ' ' << schema.m_MutationProb << ' '
                ...
                if (schema.type == "int") {
                    ...
                    output << details.min << ' ' << details.max << ' ' << details.mut_power << ' ' << details.mut_replace_prob;
        ...
        inline std::map<std::string, TraitParameters> ReadTraitParameters(std::istream &input) {
            std::size_t count = 0;
            input >> count;
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                input >> token >> std::quoted(key) >> schema.m_ImportanceCoeff >> schema.m_MutationProb >> std::quoted(schema.type) >>
                    std::quoted(schema.dep_key);
                if (token != "TraitSchema") throw std::runtime_error("trait schema map: missing TraitSchema marker.");
                schema.dep_values = ReadTraitValues(input);

                input >> token;
                if (token != "TraitDetails") throw std::runtime_error("trait schema map: missing TraitDetails marker.");
                if (schema.type == "int") {
                    IntTraitParameters details;
                    input >> details.min >> details.max >> details.mut_power >> details.mut_replace_prob;
