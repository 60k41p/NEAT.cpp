 ...
 *
 * This file has been modified from its original version by Gökalp Özcan in
 * 2026.
 *
 * Contact info:
 ...
 * Description: Implementation of the phenotype activation functions, spiking
...
namespace NEAT {
    ...
    namespace {
        ...
        void ValidateNetworkTopology(const NeuralNetwork &network) {
            ...
            if (network.m_num_inputs > neuron_count || network.m_num_outputs > neuron_count - network.m_num_inputs) {
                throw std::runtime_error("Neural network input/output dimensions exceed its neuron count");
            }
            for (const auto &connection : network.m_connections) {
                if (connection.m_source_neuron_idx < 0 || connection.m_target_neuron_idx < 0 ||
                    static_cast<std::size_t>(connection.m_source_neuron_idx) >= neuron_count ||
                    static_cast<std::size_t>(connection.m_target_neuron_idx) >= neuron_count) {
                    throw std::runtime_error("Neural network connection index is out of range");
                }
            }
        }
        ...
        double activation_derivative(const Neuron &neuron) {
            ...
            switch (neuron.m_activation_function_type) {
                ...
                case TANH_CUBIC:
                    return neuron.m_a * 3.0 * input * input * (1.0 - output * output);
                case SIGNED_GAUSS:
                ...
                case SIGNED_SINE:
                    return neuron.m_a * std::cos(input * neuron.m_a + neuron.m_b);
                case UNSIGNED_SINE:
                    return 0.5 * neuron.m_a * std::cos(input * neuron.m_a + neuron.m_b);
                case LINEAR:
                    return 1.0;
                case RELU:
        ...
        inline double EvaluateActivation(Neuron &neuron, double input) {
            ...
            switch (neuron.m_activation_function_type) {
                ...
                case MCCULLOCH_PITTS:
                    return (!neuron.m_mcp_inhibitory_veto || !neuron.m_inhibitory_input) && input >= neuron.m_spike_threshold ? 1.0 : 0.0;
                default:
                    return af_sigmoid_unsigned(input, neuron.m_a, neuron.m_b);
            }
        }
    ...
    NeuralNetwork::NeuralNetwork(bool a_Minimal) {
        if (!a_Minimal) {
            ...
            const int endpoints[][2] = {{0, 3}, {1, 3}, {2, 3}, {0, 4}, {1, 4}, {2, 4}, {4, 3}};
    ...
    void NeuralNetwork::InitRTRLMatrix() {
        if (IsSpiking()) {
            throw std::invalid_argument(
                "RTRL is not defined for non-differentiable "
                "spiking activations; use evolution or STDP");
        }
        // initializes the sensitivity cube for RTRL learning.
    ...
    void NeuralNetwork::ActivateFast() {
        ...
        for (auto &conn : m_connections) {
            conn.m_source_activation = m_neurons[conn.m_source_neuron_idx].m_activation;
        ...
        // and store the result back to nodes_activations also skip inputs since they
    ...
    void NeuralNetwork::ActivateUseInternalBias() {
        ...
        if (IsSpiking()) {
            throw std::invalid_argument(
                "Use StepSpiking for spiking networks; neuron "
                "bias is already included as tonic current");
        }
        ...
        // Loop the connections again. This time add the signals to the target
        // neurons.
    ...
    void NeuralNetwork::ActivateLeaky(double a_dtime) {
        ...
        if (!std::isfinite(a_dtime) || a_dtime < 0.0) {
            throw std::invalid_argument("Leaky activation time step must be finite and non-negative");
        }
        for (std::size_t i = m_num_inputs; i < m_neurons.size(); ++i) {
            if (!std::isfinite(m_neurons[i].m_timeconst) || m_neurons[i].m_timeconst <= 0.0) {
                throw std::domain_error("Leaky activation requires positive, finite neuron time constants");
            }
        }
        // Loop connections. Calculate each connection's output signal.
        for (auto &conn : m_connections) {
    ...
    void NeuralNetwork::RTRL_update_gradients() {
        ...
        for (std::size_t index = 0; index < m_connections.size(); ++index) {
            ...
            if (connection_indices[target][source] >= 0) {
                throw std::invalid_argument("RTRL requires unique connection endpoints");
            }
            connection_indices[target][source] = static_cast<int>(index);
            incoming[target].emplace_back(source, connection.m_weight);
        }
        ...
        for (std::size_t k = m_num_inputs; k < neuron_count; ++k) {
            ...
            for (const Connection &parameter : m_connections) {
                const std::size_t i = static_cast<std::size_t>(parameter.m_target_neuron_idx);
                ...
                if (i == k) sum += parameter.m_source_activation;
                m_neurons[k].m_sensitivity_matrix[i][j] = derivative * sum;
            }
        }
    }
    ...
    void NeuralNetwork::Flush() {
        for (auto &neuron : m_neurons) {
            ...
            neuron.m_activesum = 0;
            neuron.m_membrane_potential = neuron.m_activation_function_type == SPIKING_IZHIKEVICH ? neuron.m_izhikevich_c : neuron.m_resting_potential;
            neuron.m_last_input = 0;
            neuron.m_refractory_remaining = 0.0;
            neuron.m_adaptation = 0.0;
            neuron.m_izhikevich_recovery = neuron.m_izhikevich_b * neuron.m_membrane_potential;
    ...
    void NeuralNetwork::InputExact(const std::vector<double> &a_Inputs) {
        ...
        if (m_num_inputs > m_neurons.size()) {
            throw std::runtime_error("Neural network input count exceeds its neuron count");
        }
        for (std::size_t i = 0; i < a_Inputs.size(); ++i) m_neurons[i].m_activation = a_Inputs[i];
    }
    std::size_t NeuralNetwork::SparseRTRLStateSize() const {
        std::size_t size = 0;
        for (const auto &row : m_sparse_rtrl_sensitivities) size += row.size();
        return size;
    }
    ...
    void NeuralNetwork::SetSpikingInputMode(SpikingInputMode mode) {
        if (mode < CURRENT_INPUT || mode > POISSON_RATE_INPUT) throw std::invalid_argument("Unsupported spiking input mode");
        m_spiking_input_mode = mode;
    }

    void NeuralNetwork::SetSpikingOutputMode(SpikingOutputMode mode) {
        if (mode < SPIKE_OUTPUT || mode > MEMBRANE_POTENTIAL_OUTPUT) {
            throw std::invalid_argument("Unsupported spiking output mode");
        }
    ...
    void NeuralNetwork::SeedSpiking(std::uint64_t seed) {
        ...
        m_spiking_rng_state = seed == 0 ? UINT64_C(0x9e3779b97f4a7c15) : seed;
    }

    void NeuralNetwork::EnableSpikeRecording(bool enabled, std::size_t max_events) {
        m_record_spikes = enabled;
        m_max_recorded_spikes = max_events;
        if (!enabled)
            m_spike_history.clear();
        else if (max_events > 0 && m_spike_history.size() > max_events) {
            m_spike_history.erase(m_spike_history.begin(), m_spike_history.end() - static_cast<std::ptrdiff_t>(max_events));
        }
    }
    ...
    std::vector<double> NeuralNetwork::StepSpiking(const std::vector<double> &inputs, double time_step) {
        ...
        const auto record = [this](const SpikeEvent &event) {
            ...
            if (m_max_recorded_spikes > 0 && m_spike_history.size() > m_max_recorded_spikes) {
                const std::size_t excess = m_spike_history.size() - m_max_recorded_spikes;
                m_spike_history.erase(m_spike_history.begin(), m_spike_history.begin() + static_cast<std::ptrdiff_t>(excess));
            }
        };
        ...
        for (std::size_t i = 0; i < m_neurons.size(); ++i) {
            ...
            if (i < m_num_inputs) {
                ...
                switch (m_spiking_input_mode) {
                    ...
                    default:
                        throw std::logic_error("Invalid spiking input mode");
                }
                ...
                if (spike) {
                    ...
                    record(SpikeEvent{step_start, static_cast<int>(i), amplitude, true});
                }
            } else {
        ...
        for (auto &connection : m_connections) {
            ...
            connection.m_synaptic_current *= std::exp(-dt / connection.m_synaptic_time_constant);
            connection.m_signal = 0.0;
            connection.m_presynaptic_signal = 0.0;
            ...
            const bool event_source =
                IsSpikingActivation(source_neuron.m_activation_function_type) || (source < m_num_inputs && m_spiking_input_mode != CURRENT_INPUT);
            if (event_source && source_spikes[source]) {
                PendingSynapticEvent event;
                event.delivery_time = step_start + connection.m_synaptic_delay;
                event.amplitude = source_amplitudes[source] * connection.m_weight;
                event.source_amplitude = source_amplitudes[source];
            ...
            while (pending != connection.m_pending_events.end()) {
                if (!std::isfinite(pending->delivery_time) || !std::isfinite(pending->amplitude) || !std::isfinite(pending->source_amplitude)) {
                    throw std::domain_error("Pending synaptic events must be finite");
                }
                if (pending->delivery_time <= step_end + delivery_epsilon) {
                    connection.m_synaptic_current += pending->amplitude;
                    connection.m_signal += pending->amplitude;
                    connection.m_presynaptic_signal += pending->source_amplitude;
                    pending = connection.m_pending_events.erase(pending);
                } else {
                    ++pending;
                }
            }

            Neuron &target = m_neurons[static_cast<std::size_t>(connection.m_target_neuron_idx)];
            ...
            if (!event_source) {
                ...
                if (analog < 0.0) target.m_inhibitory_input = true;
            }
        }
        ...
        for (std::size_t i = m_num_inputs; i < m_neurons.size(); ++i) {
            ...
            if (!std::isfinite(neuron.m_rate_time_constant) || neuron.m_rate_time_constant <= 0.0) {
                throw std::domain_error(
                    "Spike-rate filters require a positive finite "
                    "time constant");
            }
            neuron.m_rate_trace *= std::exp(-dt / neuron.m_rate_time_constant);

            if (!IsSpikingActivation(neuron.m_activation_function_type)) {
                neuron.m_activation = EvaluateActivation(neuron, current);
                neuron.m_activesum = 0.0;
                continue;
            }

            if (neuron.m_activation_function_type == MCCULLOCH_PITTS) {
                if (!std::isfinite(neuron.m_spike_threshold) || !std::isfinite(neuron.m_refractory_period) || neuron.m_refractory_period < 0.0) {
                    throw std::domain_error(
                ...
                if (!std::isfinite(neuron.m_timeconst) || neuron.m_timeconst <= 0.0 || !std::isfinite(neuron.m_spike_threshold) ||
                    !std::isfinite(neuron.m_reset_potential) || !std::isfinite(neuron.m_resting_potential) || !std::isfinite(neuron.m_refractory_period) ||
                    neuron.m_refractory_period < 0.0 || !std::isfinite(neuron.m_membrane_resistance)) {
                    throw std::domain_error(
                        "LIF neurons require finite parameters and a "
                        "positive membrane time constant");
                }
                if (neuron.m_activation_function_type == SPIKING_ADAPTIVE_LIF) {
                    if (!std::isfinite(neuron.m_adaptation_time_constant) || neuron.m_adaptation_time_constant <= 0.0 ||
                        !std::isfinite(neuron.m_adaptation_increment)) {
                        throw std::domain_error(
                            "Adaptive LIF neurons require finite "
                            "adaptation parameters");
                    }
                    neuron.m_adaptation *= std::exp(-dt / neuron.m_adaptation_time_constant);
                } else {
                    neuron.m_adaptation = 0.0;
                }
                ...
                if (neuron.m_refractory_remaining > 0.0) {
                    ...
                    neuron.m_membrane_potential += (dt / neuron.m_timeconst) * (neuron.m_resting_potential - neuron.m_membrane_potential +
                                                                                neuron.m_membrane_resistance * current - neuron.m_adaptation);
                    if (neuron.m_membrane_potential >= neuron.m_spike_threshold) {
                        neuron.m_spike = true;
                        neuron.m_membrane_potential = neuron.m_reset_potential;
                        neuron.m_refractory_remaining = neuron.m_refractory_period;
                        if (neuron.m_activation_function_type == SPIKING_ADAPTIVE_LIF) {
                            neuron.m_adaptation += neuron.m_adaptation_increment;
                        }
                    }
                }
            }
            ...
            if (neuron.m_spike) {
                ...
                neuron.m_rate_trace += 1.0 / neuron.m_rate_time_constant;
                record(SpikeEvent{step_end, static_cast<int>(i), 1.0, false});
            }
            neuron.m_activesum = 0.0;
        }

        for (auto &connection : m_connections) {
            if (!connection.m_stdp_enabled) continue;
            if (!std::isfinite(connection.m_stdp_tau_plus) || connection.m_stdp_tau_plus <= 0.0 || !std::isfinite(connection.m_stdp_tau_minus) ||
                connection.m_stdp_tau_minus <= 0.0 || !std::isfinite(connection.m_stdp_plus) || connection.m_stdp_plus < 0.0 ||
                !std::isfinite(connection.m_stdp_minus) || connection.m_stdp_minus < 0.0 || !std::isfinite(connection.m_stdp_min_weight) ||
                !std::isfinite(connection.m_stdp_max_weight) || connection.m_stdp_min_weight > connection.m_stdp_max_weight) {
                throw std::domain_error(
                    "STDP requires positive trace time constants and "
                    "ordered finite weight bounds");
            }
            connection.m_stdp_pre_trace *= std::exp(-dt / connection.m_stdp_tau_plus);
            connection.m_stdp_post_trace *= std::exp(-dt / connection.m_stdp_tau_minus);
            const bool pre = m_neurons[static_cast<std::size_t>(connection.m_source_neuron_idx)].m_spike;
            const bool post = m_neurons[static_cast<std::size_t>(connection.m_target_neuron_idx)].m_spike;
            if (pre) {
                connection.m_weight -= connection.m_stdp_minus * connection.m_stdp_post_trace;
                connection.m_stdp_pre_trace += 1.0;
            }
            if (post) {
                connection.m_weight += connection.m_stdp_plus * connection.m_stdp_pre_trace;
                connection.m_stdp_post_trace += 1.0;
            }
            Clamp(connection.m_weight, connection.m_stdp_min_weight, connection.m_stdp_max_weight);
        }
    ...
    void NeuralNetwork::UpdateConnectionGeometry(bool update_delays, double conduction_velocity) {
        ...
        if (update_delays && (!std::isfinite(conduction_velocity) || conduction_velocity <= 0.0)) {
    ...
    void NeuralNetwork::RTRL_update_gradients_sparse() {
        ...
        for (std::size_t neuron = 0; neuron < neuron_count; ++neuron) {
            auto &sensitivities = m_sparse_rtrl_sensitivities[neuron];
            ...
            for (std::size_t parameter = 0; parameter < connection_count; ++parameter) {
                ...
                sensitivities[parameter] = derivative * sensitivity;
            }
        }
    }
    void NeuralNetwork::RTRL_update_error(const std::vector<double> &targets, double learning_rate) {
        ...
        if (!std::isfinite(learning_rate) || learning_rate < 0.0) {
            throw std::invalid_argument("RTRL learning rate must be finite and non-negative");
        }
        if (m_total_weight_change.size() != m_connections.size()) {
            m_total_weight_change.assign(m_connections.size(), 0.0);
        }
        for (unsigned int output = 0; output < m_num_outputs; ++output) {
            const auto &matrix = m_neurons[m_num_inputs + output].m_sensitivity_matrix;
            if (matrix.size() != m_neurons.size()) {
                throw std::runtime_error("RTRL gradients must be initialized before updating error");
            }
            for (const auto &row : matrix) {
                if (row.size() != m_neurons.size()) {
                    throw std::runtime_error("RTRL sensitivity matrix has invalid dimensions");
                }
            }
        }
        ...
        for (std::size_t connection_index = 0; connection_index < m_connections.size(); ++connection_index) {
    ...
    void NeuralNetwork::RTRL_update_error_sparse(const std::vector<double> &targets, double learning_rate) {
        ...
        for (std::size_t parameter = 0; parameter < m_connections.size(); ++parameter) {
            ...
            m_total_weight_change[parameter] += learning_rate * gradient;
        }
    ...
    void NeuralNetwork::RTRL_update_weights() {
        if (m_total_weight_change.size() != m_connections.size()) {
            throw std::runtime_error("RTRL weight state does not match the network connections");
        }
        for (std::size_t i = 0; i < m_connections.size(); ++i) {
    ...
    void NeuralNetwork::Save(const char *a_filename) {
        if (a_filename == nullptr) throw std::invalid_argument("NeuralNetwork::Save: filename is null.");
        FILE *fil = detail::OpenFile(a_filename, "w");
        if (fil == nullptr) {
            throw std::runtime_error("Cannot open neural network file for writing");
        }
        Save(fil);
        if (fclose(fil) != 0) throw std::runtime_error("NeuralNetwork::Save: failed to close output file.");
    }

    void NeuralNetwork::Save(FILE *a_file) {
        if (a_file == nullptr) throw std::invalid_argument("NeuralNetwork::Save: file is null.");
        ValidateNetworkTopology(*this);
        fprintf(a_file, "NNstart\n");
        fprintf(a_file, "%u %u\n", m_num_inputs, m_num_outputs);
        fprintf(a_file, "spiking_state %3.18f %3.18f %d %d %d %zu %llu\n", m_spiking_time, m_spiking_time_step, static_cast<int>(m_spiking_input_mode),
                static_cast<int>(m_spiking_output_mode), static_cast<int>(m_record_spikes), m_max_recorded_spikes,
                static_cast<unsigned long long>(m_spiking_rng_state));
        for (const auto &neuron : m_neurons) {
            fprintf(a_file, "neuron %d %3.18f %3.18f %3.18f %3.18f %d %3.18f\n", static_cast<int>(neuron.m_type), neuron.m_a, neuron.m_b, neuron.m_timeconst,
                    neuron.m_bias, static_cast<int>(neuron.m_activation_function_type), neuron.m_split_y);
            fprintf(a_file,
                    "spiking_neuron %3.18f %3.18f %3.18f %3.18f "
                    "%3.18f %3.18f %3.18f %3.18f %3.18f %3.18f "
                    "%3.18f %3.18f %3.18f %3.18f %3.18f %d %llu "
                    "%3.18f %3.18f %3.18f\n",
                    neuron.m_spike_threshold, neuron.m_reset_potential, neuron.m_resting_potential, neuron.m_refractory_period, neuron.m_refractory_remaining,
                    neuron.m_membrane_resistance, neuron.m_adaptation_time_constant, neuron.m_adaptation_increment, neuron.m_adaptation, neuron.m_izhikevich_a,
                    neuron.m_izhikevich_b, neuron.m_izhikevich_c, neuron.m_izhikevich_d, neuron.m_izhikevich_recovery, neuron.m_rate_trace,
                    static_cast<int>(neuron.m_spike), static_cast<unsigned long long>(neuron.m_spike_count), neuron.m_last_spike_time,
                    neuron.m_rate_time_constant, neuron.m_membrane_potential);
            fprintf(a_file, "mcp_neuron %d %d\n", static_cast<int>(neuron.m_mcp_inhibitory_veto), static_cast<int>(neuron.m_inhibitory_input));
            fprintf(a_file,
        ...
        for (const auto &conn : m_connections) {
            fprintf(a_file, "connection %d %d %3.18f %d %3.18f %3.18f\n", conn.m_source_neuron_idx, conn.m_target_neuron_idx, conn.m_weight,
                    static_cast<int>(conn.m_recur_flag), conn.m_hebb_rate, conn.m_hebb_pre_rate);
            fprintf(a_file,
                    "spiking_connection %3.18f %3.18f %3.18f %d "
                    "%3.18f %3.18f %3.18f %3.18f %3.18f %3.18f "
                    "%3.18f %3.18f\n",
                    conn.m_synaptic_delay, conn.m_synaptic_time_constant, conn.m_synaptic_current, static_cast<int>(conn.m_stdp_enabled), conn.m_stdp_plus,
                    conn.m_stdp_minus, conn.m_stdp_tau_plus, conn.m_stdp_tau_minus, conn.m_stdp_pre_trace, conn.m_stdp_post_trace, conn.m_stdp_min_weight,
                    conn.m_stdp_max_weight);
            fprintf(a_file, "spatial_connection %3.18f\n", conn.m_length);
        }
    ...
    bool NeuralNetwork::Load(std::ifstream &a_DataFile) {
        ...
        while (a_DataFile >> t_str && t_str != "NNend") {
            if (t_str == "spiking_state") {
                ...
                a_DataFile >> m_spiking_time >> m_spiking_time_step >> input_mode >> output_mode >> record >> m_max_recorded_spikes >> rng_state;
                m_spiking_input_mode = static_cast<SpikingInputMode>(input_mode);
                m_spiking_output_mode = static_cast<SpikingOutputMode>(output_mode);
                m_record_spikes = record != 0;
                m_spiking_rng_state = static_cast<std::uint64_t>(rng_state);
            } else if (t_str == "neuron") {
                Neuron t_n;
                int t_type, t_aftype;
                a_DataFile >> t_type >> t_n.m_a >> t_n.m_b >> t_n.m_timeconst >> t_n.m_bias;
                ...
                last_neuron = static_cast<int>(m_neurons.size()) - 1;
            } else if (t_str == "spiking_neuron") {
                if (last_neuron < 0) {
                    Clear();
                    return false;
                }
                Neuron &neuron = m_neurons[static_cast<std::size_t>(last_neuron)];
                int spike = 0;
                unsigned long long spike_count = 0;
                a_DataFile >> neuron.m_spike_threshold >> neuron.m_reset_potential >> neuron.m_resting_potential >> neuron.m_refractory_period >>
                    neuron.m_refractory_remaining >> neuron.m_membrane_resistance >> neuron.m_adaptation_time_constant >> neuron.m_adaptation_increment >>
                    neuron.m_adaptation >> neuron.m_izhikevich_a >> neuron.m_izhikevich_b >> neuron.m_izhikevich_c >> neuron.m_izhikevich_d >>
                    neuron.m_izhikevich_recovery >> neuron.m_rate_trace >> spike >> spike_count >> neuron.m_last_spike_time >> neuron.m_rate_time_constant >>
                    neuron.m_membrane_potential;
                neuron.m_spike = spike != 0;
                neuron.m_spike_count = static_cast<std::uint64_t>(spike_count);
                ...
                last_connection = static_cast<int>(m_connections.size()) - 1;
            } else if (t_str == "spiking_connection") {
                if (last_connection < 0) {
                    Clear();
                    return false;
                }
                Connection &connection = m_connections[static_cast<std::size_t>(last_connection)];
                int stdp = 0;
                a_DataFile >> connection.m_synaptic_delay >> connection.m_synaptic_time_constant >> connection.m_synaptic_current >> stdp >>
                    connection.m_stdp_plus >> connection.m_stdp_minus >> connection.m_stdp_tau_plus >> connection.m_stdp_tau_minus >>
                    connection.m_stdp_pre_trace >> connection.m_stdp_post_trace >> connection.m_stdp_min_weight >> connection.m_stdp_max_weight;
                ...
                a_DataFile >> m_connections[static_cast<std::size_t>(last_connection)].m_length;
            }
        }
    ...
    bool NeuralNetwork::Load(const char *a_filename) {
        if (a_filename == nullptr) return false;
        std::ifstream t_DataFile(a_filename);
        if (!t_DataFile) return false;
    ...
    std::string NeuralNetwork::Serialize() const {
        ...
        for (const auto &neuron : m_neurons) {
            output << "Neuron " << neuron.m_activesum << ' ' << neuron.m_activation << ' ' << neuron.m_a << ' ' << neuron.m_b << ' ' << neuron.m_timeconst
                   << ' ' << neuron.m_bias << ' ' << neuron.m_membrane_potential << ' ' << neuron.m_last_input << ' '
                   << static_cast<int>(neuron.m_activation_function_type) << ' ' << neuron.m_x << ' ' << neuron.m_y << ' ' << neuron.m_z << ' ' << neuron.m_sx
                   << ' ' << neuron.m_sy << ' ' << neuron.m_sz << ' ' << neuron.m_split_y << ' ' << static_cast<int>(neuron.m_type) << ' '
                   << neuron.m_spike_threshold << ' ' << neuron.m_reset_potential << ' ' << neuron.m_resting_potential << ' ' << neuron.m_refractory_period
                   << ' ' << neuron.m_refractory_remaining << ' ' << neuron.m_membrane_resistance << ' ' << neuron.m_adaptation_time_constant << ' '
                   << neuron.m_adaptation_increment << ' ' << neuron.m_adaptation << ' ' << neuron.m_izhikevich_a << ' ' << neuron.m_izhikevich_b << ' '
                   << neuron.m_izhikevich_c << ' ' << neuron.m_izhikevich_d << ' ' << neuron.m_izhikevich_recovery << ' ' << static_cast<int>(neuron.m_spike)
        ...
        for (const auto &connection : m_connections) {
            output << "Connection " << connection.m_source_neuron_idx << ' ' << connection.m_target_neuron_idx << ' ' << connection.m_weight << ' '
                   << connection.m_signal << ' ' << connection.m_source_activation << ' ' << static_cast<int>(connection.m_recur_flag) << ' '
                   << connection.m_hebb_rate << ' ' << connection.m_hebb_pre_rate << ' ' << connection.m_synaptic_delay << ' '
                   << connection.m_synaptic_time_constant << ' ' << connection.m_synaptic_current << ' ' << connection.m_presynaptic_signal << ' '
                   << static_cast<int>(connection.m_stdp_enabled) << ' ' << connection.m_stdp_plus << ' ' << connection.m_stdp_minus << ' '
                   << connection.m_stdp_tau_plus << ' ' << connection.m_stdp_tau_minus << ' ' << connection.m_stdp_pre_trace << ' '
                   << connection.m_stdp_post_trace << ' ' << connection.m_stdp_min_weight << ' ' << connection.m_stdp_max_weight << ' ' << connection.m_length
                   << '\n';
            output << "PendingEvents " << connection.m_pending_events.size();
            for (const auto &event : connection.m_pending_events) {
                output << ' ' << event.delivery_time << ' ' << event.amplitude << ' ' << event.source_amplitude;
            }
            output << '\n';
        }
        ...
        for (const auto &event : m_spike_history) {
            output << "SpikeEvent " << event.time << ' ' << event.neuron_index << ' ' << event.amplitude << ' ' << static_cast<int>(event.input) << '\n';
        }
    ...
    NeuralNetwork NeuralNetwork::Deserialize(const std::string &data) {
        ...
        if (token != "NeuralNetworkFormat") {
            ...
            try {
                network.m_num_inputs = static_cast<unsigned int>(std::stoul(token));
            } catch (const std::exception &) {
                throw std::runtime_error("NeuralNetwork::Deserialize: malformed header.");
            }
            ...
            for (std::size_t i = 0; i < neuron_count; ++i) {
                Neuron neuron;
                int type, activation;
                input >> type >> neuron.m_a >> neuron.m_b >> neuron.m_timeconst >> neuron.m_bias >> activation >> neuron.m_split_y;
                neuron.m_type = static_cast<NeuronType>(type);
                neuron.m_activation_function_type = static_cast<ActivationFunction>(activation);
            ...
            for (std::size_t i = 0; i < connection_count; ++i) {
                Connection connection;
                int recurrent = 0;
                input >> connection.m_source_neuron_idx >> connection.m_target_neuron_idx >> connection.m_weight >> recurrent >> connection.m_hebb_rate >>
                    connection.m_hebb_pre_rate;
                connection.m_recur_flag = recurrent != 0;
                network.m_connections.push_back(connection);
            }
            if (!input) throw std::runtime_error("NeuralNetwork::Deserialize: malformed legacy data.");
        ...
        int version = 0;
        input >> version;
        if (version < 2 || version > 7) throw std::runtime_error("NeuralNetwork::Deserialize: unsupported format.");
        input >> token;
        if (token != "State") throw std::runtime_error("NeuralNetwork::Deserialize: missing State marker.");
        input >> network.m_num_inputs >> network.m_num_outputs >> network.m_total_error;
        if (version >= 5) {
            int input_mode = 0;
            int output_mode = 0;
            int record_spikes = 0;
            input >> network.m_spiking_time >> network.m_spiking_time_step >> input_mode >> output_mode >> record_spikes >> network.m_max_recorded_spikes >>
                network.m_spiking_rng_state;
            network.m_spiking_input_mode = static_cast<SpikingInputMode>(input_mode);
            network.m_spiking_output_mode = static_cast<SpikingOutputMode>(output_mode);
            network.m_record_spikes = record_spikes != 0;
            if (network.m_spiking_input_mode < CURRENT_INPUT || network.m_spiking_input_mode > POISSON_RATE_INPUT ||
                network.m_spiking_output_mode < SPIKE_OUTPUT || network.m_spiking_output_mode > MEMBRANE_POTENTIAL_OUTPUT) {
                throw std::runtime_error("NeuralNetwork::Deserialize: invalid spiking mode.");
            }
        }
        ...
        for (std::size_t i = 0; i < count; ++i) {
            input >> token;
            if (token != "Neuron") throw std::runtime_error("NeuralNetwork::Deserialize: missing Neuron marker.");
            Neuron neuron;
            int activation = 0;
            int type = 0;
            input >> neuron.m_activesum >> neuron.m_activation >> neuron.m_a >> neuron.m_b >> neuron.m_timeconst >> neuron.m_bias >>
                neuron.m_membrane_potential;
            if (version >= 4) input >> neuron.m_last_input;
            input >> activation >> neuron.m_x >> neuron.m_y >> neuron.m_z >> neuron.m_sx >> neuron.m_sy >> neuron.m_sz >> neuron.m_split_y >> type;
            neuron.m_activation_function_type = static_cast<ActivationFunction>(activation);
            neuron.m_type = static_cast<NeuronType>(type);
            if (version >= 5) {
                int spike = 0;
                input >> neuron.m_spike_threshold >> neuron.m_reset_potential >> neuron.m_resting_potential >> neuron.m_refractory_period >>
                    neuron.m_refractory_remaining >> neuron.m_membrane_resistance >> neuron.m_adaptation_time_constant >> neuron.m_adaptation_increment >>
                    neuron.m_adaptation >> neuron.m_izhikevich_a >> neuron.m_izhikevich_b >> neuron.m_izhikevich_c >> neuron.m_izhikevich_d >>
                    neuron.m_izhikevich_recovery >> spike >> neuron.m_spike_count >> neuron.m_last_spike_time >> neuron.m_rate_trace >>
            ...
            std::size_t rows = 0;
            input >> token >> rows;
            if (token != "Sensitivity") throw std::runtime_error("NeuralNetwork::Deserialize: missing sensitivity state.");
        ...
        input >> token >> count;
        if (token != "Connections") throw std::runtime_error("NeuralNetwork::Deserialize: missing Connections marker.");
        network.m_connections.reserve(count);
        for (std::size_t i = 0; i < count; ++i) {
            input >> token;
            if (token != "Connection") throw std::runtime_error("NeuralNetwork::Deserialize: missing Connection marker.");
            Connection connection;
            int recurrent = 0;
            input >> connection.m_source_neuron_idx >> connection.m_target_neuron_idx >> connection.m_weight >> connection.m_signal;
            if (version >= 3) input >> connection.m_source_activation;
            input >> recurrent >> connection.m_hebb_rate >> connection.m_hebb_pre_rate;
            connection.m_recur_flag = recurrent != 0;
            if (version >= 5) {
                int stdp = 0;
                input >> connection.m_synaptic_delay >> connection.m_synaptic_time_constant >> connection.m_synaptic_current;
                if (version >= 6) input >> connection.m_presynaptic_signal;
                input >> stdp >> connection.m_stdp_plus >> connection.m_stdp_minus >> connection.m_stdp_tau_plus >> connection.m_stdp_tau_minus >>
                    connection.m_stdp_pre_trace >> connection.m_stdp_post_trace >> connection.m_stdp_min_weight >> connection.m_stdp_max_weight;
                if (version >= 7) input >> connection.m_length;
                connection.m_stdp_enabled = stdp != 0;
        ...
        if (version >= 4) {
            std::size_t rows = 0;
            input >> token >> rows;
            if (token != "SparseRTRL") throw std::runtime_error("NeuralNetwork::Deserialize: missing sparse RTRL state.");
        ...
        if (version >= 5) {
            ...
            if (token != "SpikeHistory") {
                throw std::runtime_error("NeuralNetwork::Deserialize: missing spike history.");
            }
            network.m_spike_history.resize(event_count);
            for (auto &event : network.m_spike_history) {
                int is_input = 0;
                input >> token >> event.time >> event.neuron_index >> event.amplitude >> is_input;
                if (token != "SpikeEvent") {
                    throw std::runtime_error(
        ...
        input >> token;
        if (token != "NeuralNetworkEnd" || !input) throw std::runtime_error("NeuralNetwork::Deserialize: malformed network data.");
        ValidateNetworkTopology(network);
        return network;
    }
}  // namespace NEAT
