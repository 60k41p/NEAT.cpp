...
namespace NEAT {
    ...
    void Scale(std::vector<Real> &a_Values, const Real a_tr_min, const Real a_tr_max) {
        if (a_Values.empty()) return;

        Real t_max = 0.0, t_min = 0.0;
        GetMaxMin(a_Values, t_min, t_max);
        std::vector<Real> t_ValuesScaled;
        t_ValuesScaled.reserve(a_Values.size());
        for (std::vector<Real>::const_iterator t_It = a_Values.begin(); t_It != a_Values.end(); ++t_It) {
