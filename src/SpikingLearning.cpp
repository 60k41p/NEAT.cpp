...
/* File:        SpikingLearning.cpp
 * Description: Implementation of eligibility-propagation (e-prop) online learning for spiking networks.
 */
...
namespace {
    ...
    bool ReadMarker(std::istream &input, const char *expected) {
        std::string marker;
        return static_cast<bool>(input >> marker) && marker == expected;
    }
}  // namespace
...
namespace NEAT {
    void EPropLearner::ValidateConfig() const {
        ...
        if (!positive_finite(m_config.learning_rate) || !positive_finite(m_config.surrogate_scale) || !positive_finite(m_config.surrogate_dampening) ||
            ...
            m_config.min_weight > m_config.max_weight || m_config.update_interval == 0 || !positive_finite(m_config.huber_delta)) {
            throw std::invalid_argument("e-prop configuration contains invalid ranges");
        }
        if (m_config.optimizer < EPROP_ADAMW || m_config.optimizer > EPROP_SGD || m_config.feedback_mode < EPROP_RANDOM_FEEDBACK ||
            m_config.feedback_mode > EPROP_UNIFORM_FEEDBACK || m_config.surrogate < EPROP_FAST_SIGMOID || m_config.surrogate > EPROP_ARCTAN ||
            m_config.loss < EPROP_MEAN_SQUARED_ERROR || m_config.loss > EPROP_HUBER_LOSS) {
            throw std::invalid_argument("e-prop configuration contains an unsupported mode");
        }
        if (!std::isfinite(m_config.adam_beta1) || m_config.adam_beta1 < 0.0 || m_config.adam_beta1 >= 1.0 || !std::isfinite(m_config.adam_beta2) ||
            m_config.adam_beta2 < 0.0 || m_config.adam_beta2 >= 1.0 || !positive_finite(m_config.adam_epsilon)) {
            throw std::invalid_argument("e-prop AdamW parameters are invalid");
        }
    }

    void EPropLearner::ValidateTopology(const NeuralNetwork &network) const {
        if (!IsInitialized()) {
            throw std::logic_error("e-prop learner is not initialized");
        }
        if (network.m_neurons.size() != m_neuron_count || network.NumInputs() != m_input_count || network.NumOutputs() != m_output_count ||
            network.m_connections.size() != m_connection_state.size()) {
            throw std::logic_error("e-prop topology changed; call Initialize again");
        }
        if (network.NumInputs() + network.NumOutputs() > network.m_neurons.size()) {
            throw std::invalid_argument("e-prop network dimensions are invalid");
        }
        for (std::size_t i = 0; i < m_sources.size(); ++i) {
            const Connection &connection = network.m_connections[i];
            if (connection.m_source_neuron_idx != m_sources[i] || connection.m_target_neuron_idx != m_targets[i]) {
        ...
        for (std::size_t i = 0; i < m_neuron_count; ++i) {
            if (static_cast<int>(network.m_neurons[i].m_type) != m_neuron_types[i] ||
                static_cast<int>(network.m_neurons[i].m_activation_function_type) != m_activation_types[i]) {
                throw std::logic_error(
        ...
        if (!m_config.allow_stdp) {
            const bool stdp_enabled =
                std::any_of(network.m_connections.begin(), network.m_connections.end(), [](const Connection &connection) { return connection.m_stdp_enabled; });
            if (stdp_enabled) {
                throw std::logic_error(
    ...
    bool EPropLearner::IsTrainable(const NeuralNetwork &network, std::size_t connection_index) const {
        const Connection &connection = network.m_connections[connection_index];
        ...
        if (source >= network.m_neurons.size() || target >= network.m_neurons.size() || target < network.NumInputs() ||
            !IsSpikingActivation(network.m_neurons[target].m_activation_function_type)) {
            return false;
        }
        if (connection.m_recur_flag && !m_config.train_recurrent_connections) {
            return false;
        }
        if (source < network.NumInputs() && !m_config.train_input_connections) {
            return false;
        }
        const std::size_t output_begin = network.NumInputs();
        const std::size_t output_end = output_begin + network.NumOutputs();
        if (target >= output_begin && target < output_end) return m_config.train_output_connections;
    ...
    Real EPropLearner::SurrogateDerivative(const Neuron &neuron) const {
        ...
        switch (m_config.surrogate) {
            ...
            case EPROP_ARCTAN:
                ...
                break;
            default:
                throw std::logic_error("Unsupported e-prop surrogate");
        }
        return m_config.surrogate_dampening * shape / voltage_scale;
    }

    void EPropLearner::Initialize(const NeuralNetwork &network) {
        ValidateConfig();
        if (!network.IsSpiking()) {
            throw std::invalid_argument("e-prop requires a spiking neural network");
        }
        if (network.NumInputs() + network.NumOutputs() > network.m_neurons.size() || network.NumOutputs() == 0) {
            throw std::invalid_argument("e-prop requires valid non-empty outputs");
        }
        m_neuron_count = network.m_neurons.size();
        m_input_count = network.NumInputs();
        m_output_count = network.NumOutputs();
        m_connection_state.assign(network.m_connections.size(), EPropConnectionState{});
        m_sources.clear();
        m_targets.clear();
        ...
        for (const Neuron &neuron : network.m_neurons) {
            m_neuron_types.push_back(static_cast<int>(neuron.m_type));
            m_activation_types.push_back(static_cast<int>(neuron.m_activation_function_type));
        }
        for (const Connection &connection : network.m_connections) {
            if (connection.m_source_neuron_idx < 0 || connection.m_target_neuron_idx < 0 ||
                static_cast<std::size_t>(connection.m_source_neuron_idx) >= m_neuron_count ||
                static_cast<std::size_t>(connection.m_target_neuron_idx) >= m_neuron_count) {
                throw std::invalid_argument("e-prop connection endpoint is invalid");
            }
            m_sources.push_back(connection.m_source_neuron_idx);
            m_targets.push_back(connection.m_target_neuron_idx);
        }
    ...
    void EPropLearner::RefreshFeedback(const NeuralNetwork &network) {
        if (network.m_neurons.size() != m_neuron_count || network.NumInputs() != m_input_count || network.NumOutputs() != m_output_count) {
            throw std::logic_error("e-prop feedback topology does not match learner");
        }
        ...
        for (std::size_t neuron = network.NumInputs(); neuron < m_neuron_count; ++neuron) {
            for (std::size_t output = 0; output < m_output_count; ++output) {
                ...
                } else if (m_config.feedback_mode == EPROP_UNIFORM_FEEDBACK) {
                    value = scale;
                } else {
                    for (const Connection &connection : network.m_connections) {
                        if (connection.m_source_neuron_idx == static_cast<int>(neuron) &&
                            connection.m_target_neuron_idx == static_cast<int>(output_begin + output)) {
                            value += connection.m_weight;
                        }
                    }
                }
                m_feedback[neuron * m_output_count + output] = value;
            }
        }
    }
    ...
    void EPropLearner::ZeroGradients() {
        for (auto &state : m_connection_state) state.gradient = 0.0;
        m_accumulated_steps = 0;
        m_last_gradient_norm = 0.0;
        m_last_updated_connections = 0;
    }
    ...
    void EPropLearner::AccumulateDirectSignals(NeuralNetwork &network, const std::vector<Real> &neuron_signals, Real time_step) {
        ...
        for (std::size_t index = 0; index < network.m_connections.size(); ++index) {
            Connection &connection = network.m_connections[index];
            EPropConnectionState &state = m_connection_state[index];
            const std::size_t source = static_cast<std::size_t>(connection.m_source_neuron_idx);
            const std::size_t target = static_cast<std::size_t>(connection.m_target_neuron_idx);
            const Neuron &source_neuron = network.m_neurons[source];
            const Neuron &target_neuron = network.m_neurons[target];
            const bool event_source = IsSpikingActivation(source_neuron.m_activation_function_type) ||
                                      (source < network.NumInputs() && network.GetSpikingInputMode() != CURRENT_INPUT);

            if (!std::isfinite(connection.m_synaptic_time_constant) || connection.m_synaptic_time_constant <= 0.0) {
                throw std::domain_error("e-prop requires positive synaptic time constants");
            }
            if (event_source) {
                state.synaptic_trace *= std::exp(-dt / connection.m_synaptic_time_constant);
                state.synaptic_trace += connection.m_presynaptic_signal;
            } else {
                state.synaptic_trace = source_neuron.m_activation;
            }

            if (!IsTrainable(network, index)) continue;
            if (!std::isfinite(target_neuron.m_timeconst) || target_neuron.m_timeconst <= 0.0 || !std::isfinite(target_neuron.m_membrane_resistance)) {
            ...
            if (target_neuron.m_activation_function_type == SPIKING_ADAPTIVE_LIF) {
                if (!std::isfinite(target_neuron.m_adaptation_time_constant) || target_neuron.m_adaptation_time_constant <= 0.0 ||
                    !std::isfinite(target_neuron.m_adaptation_increment)) {
                    throw std::domain_error(
                        "e-prop requires valid adaptive-LIF "
                        "parameters");
                }
                voltage_eligibility -= membrane_fraction * old_adaptation;
                adaptation_eligibility = std::exp(-dt / target_neuron.m_adaptation_time_constant) * old_adaptation +
                                         target_neuron.m_adaptation_increment * surrogate * voltage_eligibility;
            } else if (target_neuron.m_activation_function_type == SPIKING_IZHIKEVICH) {
            ...
            if (target >= output_begin && target < output_end) {
                switch (network.GetSpikingOutputMode()) {
                    case SPIKE_OUTPUT:
                        state.readout_eligibility = spike_eligibility;
                        break;
                    case FILTERED_SPIKE_OUTPUT:
                        state.readout_eligibility *= std::exp(-dt / target_neuron.m_rate_time_constant);
                        state.readout_eligibility += spike_eligibility / target_neuron.m_rate_time_constant;
                        break;
                    case FIRING_RATE_OUTPUT:
                        state.readout_eligibility += spike_eligibility;
                        break;
                    case MEMBRANE_POTENTIAL_OUTPUT:
                        state.readout_eligibility = voltage_eligibility;
                        break;
                    default:
                        throw std::logic_error("Unsupported e-prop output decoder");
                }
                eligibility = state.readout_eligibility;
                if (network.GetSpikingOutputMode() == FIRING_RATE_OUTPUT && network.SpikingTime() > 0.0) {
    ...
    void EPropLearner::AccumulateLearningSignals(NeuralNetwork &network, const std::vector<Real> &learning_signals, Real time_step) {
        ...
        if (learning_signals.size() == m_output_count) {
            ...
            std::copy(learning_signals.begin(), learning_signals.end(), direct.begin() + static_cast<std::ptrdiff_t>(network.NumInputs()));
        ...
        AccumulateDirectSignals(network, direct, time_step);
    }

    EPropStepResult EPropLearner::ApplyGradients(NeuralNetwork &network) {
        ValidateConfig();
        ValidateTopology(network);
        ...
        for (std::size_t index = 0; index < m_connection_state.size(); ++index) {
            ...
            if (m_config.optimizer == EPROP_ADAMW) {
                ...
                connection.m_weight *= 1.0 - m_config.learning_rate * m_config.weight_decay;
                connection.m_weight -= m_config.learning_rate * gradient;
            }
            connection.m_weight = std::clamp(connection.m_weight, m_config.min_weight, m_config.max_weight);
            state.gradient = 0.0;
            ++updated;
        }
        m_accumulated_steps = 0;
        m_last_gradient_norm = gradient_norm * clip_scale;
    ...
    EPropStepResult EPropLearner::TrainStep(NeuralNetwork &network, const std::vector<Real> &inputs, const std::vector<Real> &targets, Real time_step) {
        ...
        if (m_accumulated_steps >= m_config.update_interval) {
            ...
            result.gradient_norm = update.gradient_norm;
            result.updated_connections = update.updated_connections;
            result.update_applied = true;
        }
        return result;
    }
    ...
    EPropSequenceResult EPropLearner::TrainSequence(NeuralNetwork &network,
        ...
        for (std::size_t step = 0; step < inputs.size(); ++step) {
            EPropStepResult current = TrainStep(network, inputs[step], targets[step], time_step);
            result.outputs.push_back(std::move(current.outputs));
            result.losses.push_back(current.loss);
            result.mean_loss += current.loss;
            if (current.update_applied) {
                ++result.optimizer_updates;
                result.final_gradient_norm = current.gradient_norm;
                result.updated_connections += current.updated_connections;
            }
        }
        if (!inputs.empty()) {
        ...
        if (apply_final_update && m_accumulated_steps > 0) {
            ...
            ++result.optimizer_updates;
            result.final_gradient_norm = final_update.gradient_norm;
            result.updated_connections += final_update.updated_connections;
        }
        return result;
    }
    ...
    std::string EPropLearner::Serialize() const {
        ...
        output << "Config " << m_config.learning_rate << ' ' << static_cast<int>(m_config.optimizer) << ' ' << static_cast<int>(m_config.feedback_mode) << ' '
               ...
               << m_config.surrogate_dampening << ' ' << m_config.gradient_clip_norm << ' ' << m_config.weight_decay << ' ' << m_config.adam_beta1 << ' '
               << m_config.adam_beta2 << ' ' << m_config.adam_epsilon << ' ' << m_config.huber_delta << ' ' << m_config.min_weight << ' ' << m_config.max_weight
               << ' ' << m_config.update_interval << ' ' << m_config.random_seed << ' ' << static_cast<int>(m_config.train_input_connections) << ' '
               << static_cast<int>(m_config.train_hidden_connections) << ' ' << static_cast<int>(m_config.train_output_connections) << ' '
               << static_cast<int>(m_config.train_recurrent_connections) << ' ' << static_cast<int>(m_config.allow_stdp) << '\n';
        output << "State " << m_neuron_count << ' ' << m_input_count << ' ' << m_output_count << ' ' << m_connection_state.size() << ' ' << m_accumulated_steps
               << ' ' << m_optimizer_step << ' ' << m_last_gradient_norm << ' ' << m_last_updated_connections << '\n';
        output << "Endpoints " << m_sources.size() << '\n';
        for (std::size_t i = 0; i < m_sources.size(); ++i) output << m_sources[i] << ' ' << m_targets[i] << '\n';
        ...
        for (const auto &state : m_connection_state) {
            output << state.synaptic_trace << ' ' << state.voltage_eligibility << ' ' << state.adaptation_eligibility << ' ' << state.readout_eligibility << ' '
                   << state.gradient << ' ' << state.first_moment << ' ' << state.second_moment << '\n';
        }
        output << "End\n";
        return output.str();
    }

    EPropLearner EPropLearner::Deserialize(const std::string &data) {
        std::istringstream input(data);
        if (!ReadMarker(input, "EPropFormat")) {
            throw std::runtime_error("EPropLearner::Deserialize: missing header");
        }
        int version = 0;
        if (!(input >> version) || version != 1 || !ReadMarker(input, "Config")) {
            throw std::runtime_error("EPropLearner::Deserialize: unsupported format");
        }
        ...
        int allow_stdp = 0;
        if (!(input >> learner.m_config.learning_rate >> optimizer >> feedback >> surrogate >> loss >> learner.m_config.surrogate_scale >>
              learner.m_config.surrogate_dampening >> learner.m_config.gradient_clip_norm >> learner.m_config.weight_decay >> learner.m_config.adam_beta1 >>
              learner.m_config.adam_beta2 >> learner.m_config.adam_epsilon >> learner.m_config.huber_delta >> learner.m_config.min_weight >>
              learner.m_config.max_weight >> learner.m_config.update_interval >> learner.m_config.random_seed >> train_input >> train_hidden >> train_output >>
              train_recurrent >> allow_stdp)) {
            throw std::runtime_error("EPropLearner::Deserialize: malformed config");
        }
        learner.m_config.optimizer = static_cast<EPropOptimizer>(optimizer);
        learner.m_config.feedback_mode = static_cast<EPropFeedbackMode>(feedback);
        learner.m_config.surrogate = static_cast<EPropSurrogate>(surrogate);
        learner.m_config.loss = static_cast<EPropLoss>(loss);
        learner.m_config.train_input_connections = train_input != 0;
        learner.m_config.train_hidden_connections = train_hidden != 0;
        learner.m_config.train_output_connections = train_output != 0;
        learner.m_config.train_recurrent_connections = train_recurrent != 0;
        learner.m_config.allow_stdp = allow_stdp != 0;
        learner.ValidateConfig();

        std::size_t connection_count = 0;
        if (!ReadMarker(input, "State") ||
            !(input >> learner.m_neuron_count >> learner.m_input_count >> learner.m_output_count >> connection_count >> learner.m_accumulated_steps >>
              learner.m_optimizer_step >> learner.m_last_gradient_norm >> learner.m_last_updated_connections)) {
            throw std::runtime_error("EPropLearner::Deserialize: malformed state");
        }
        std::size_t endpoint_count = 0;
        if (!ReadMarker(input, "Endpoints") || !(input >> endpoint_count) || endpoint_count != connection_count) {
            throw std::runtime_error("EPropLearner::Deserialize: malformed endpoints");
        }
        learner.m_sources.resize(endpoint_count);
        learner.m_targets.resize(endpoint_count);
        for (std::size_t i = 0; i < endpoint_count; ++i) {
            if (!(input >> learner.m_sources[i] >> learner.m_targets[i])) {
                throw std::runtime_error("EPropLearner::Deserialize: malformed endpoint");
            }
        }
        std::size_t neuron_signature_count = 0;
        if (!ReadMarker(input, "Neurons") || !(input >> neuron_signature_count) || neuron_signature_count != learner.m_neuron_count) {
            throw std::runtime_error(
                "EPropLearner::Deserialize: malformed neuron "
                "signature");
        }
        learner.m_neuron_types.resize(neuron_signature_count);
        learner.m_activation_types.resize(neuron_signature_count);
        for (std::size_t i = 0; i < neuron_signature_count; ++i) {
            if (!(input >> learner.m_neuron_types[i] >> learner.m_activation_types[i])) {
                throw std::runtime_error(
                    "EPropLearner::Deserialize: malformed neuron "
                    "signature value");
            }
        ...
        if (!ReadMarker(input, "Feedback") || !(input >> feedback_count)) {
            throw std::runtime_error("EPropLearner::Deserialize: malformed feedback");
        }
        if (feedback_count != learner.m_neuron_count * learner.m_output_count) {
        ...
        if (!ReadMarker(input, "Connections") || !(input >> state_count) || state_count != connection_count) {
            throw std::runtime_error("EPropLearner::Deserialize: malformed connection state");
        }
        learner.m_connection_state.resize(state_count);
        for (auto &state : learner.m_connection_state) {
            if (!(input >> state.synaptic_trace >> state.voltage_eligibility >> state.adaptation_eligibility >> state.readout_eligibility >> state.gradient >>
                  state.first_moment >> state.second_moment) ||
                !std::isfinite(state.synaptic_trace) || !std::isfinite(state.voltage_eligibility) || !std::isfinite(state.adaptation_eligibility) ||
                !std::isfinite(state.readout_eligibility) || !std::isfinite(state.gradient) || !std::isfinite(state.first_moment) ||
                !std::isfinite(state.second_moment)) {
                throw std::runtime_error("EPropLearner::Deserialize: malformed connection");
            }
        }
        if (!ReadMarker(input, "End")) {
            throw std::runtime_error("EPropLearner::Deserialize: missing terminator");
        }
        std::string trailing;
        if (input >> trailing) {
            throw std::runtime_error("EPropLearner::Deserialize: trailing data");
        }
        return learner;
    }
}  // namespace NEAT
