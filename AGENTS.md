...
- Dead/disabled code is purged on sight: the old `#if 0`'d `MutateGenome` path in `src/Species.cpp`, commented-out code blocks, the `GetConnectionLenght` typo alias and the src-unused `RoundUnderOffset`/`Abs` helpers were all removed in 2026. (ES-HyperNEAT was formerly `#if 0`'d as well; the MultiNEAT v2 port replaced it with a live implementation.)
- Most `Parameters` knobs are consumed by the evolution loop (selection/crossover/weight-mutation/representative/offspring/threshold/scaling modes, `EliteFraction` in `src/Species.cpp`, `StagnationPenalty` in `src/Species.cpp`, competitive-coevolution `DetectCompetetiveCoevolutionStagnation`/`KillWorst*` in `src/Population.cpp`, adaptive mutation, the link enable bit with `MutateToggleEnableProb`/`DisabledGeneInheritRate`, and the HyperNEAT `Leo`/`LeoSeed`/`GeometrySeed` knobs wired through the seed genome and both HyperNEAT builders). Still-unimplemented knobs: the 2D-loop `Depth`/`Qtree_Z` (used only by the 3D octree path; `Parameters::Validate` still requires `Depth > 0` even for 2D). README documents each; verify a knob is actually consumed before assuming it works.
- Single flat namespace `NEAT`, headers in `src/` (public API: Genome, Population, NeuralNetwork, Parameters, Substrate, Traits).
- Serialization (`.NEAT` files) is hand-rolled text parsing in Save/Load methods — format changes break `tests/data/minimal.NEAT` fixtures and backward compat. Versioned string checkpoints (`GenomeFormat 5`, `SpeciesFormat 2`, `PopulationFormat 2` via `Serialize`/`Deserialize`) are the forward-compatible path.
