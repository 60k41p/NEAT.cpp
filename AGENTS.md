...
- Most `Parameters` knobs are consumed by the evolution loop (selection/crossover/weight-mutation/representative/offspring/threshold/scaling modes, `EliteFraction` in `src/Species.cpp`, `StagnationPenalty` in `src/Species.cpp`, competitive-coevolution `DetectCompetetiveCoevolutionStagnation`/`KillWorst*` in `src/Population.cpp`, adaptive mutation). Still-unimplemented knobs: `LeoSeed`/`GeometrySeed` (validated/serialized but never read by either HyperNEAT builder), plain-HyperNEAT `Leo` (only the ES path consumes it), and the 2D-loop `Depth`/`Qtree_Z` (used only by the 3D octree path; `Parameters::Validate` still requires `Depth > 0` even for 2D). README documents each; verify a knob is actually consumed before assuming it works.
