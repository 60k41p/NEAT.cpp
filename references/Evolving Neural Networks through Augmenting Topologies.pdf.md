...
c(cid:141)2002 by the Massachusetts Institute of Technology
...
space is explored through the crossover of network weight vectors and through the mu-
...
permuting over different topologies?

The answers provided thus far are inconclusive. Some have argued that network
complexity can affect the speed and accuracy of learning (Zhang and M ¨uhlenbein,
1993). Although this assertion is true for the backpropagation algorithm, it is not clear
whether it applies when weights are being optimized by evolution and not backpropa-
gation.
...
Gruau et al. supported their argument by evolving the topology and weights of an ar-
tiﬁcial neural network that solved the hardest pole-balancing benchmark problem to
date. However, later results suggested that structure was not necessary to solve the dif-
...
Evolving structure incrementally presents several technical challenges: (1) Is there
a genetic representation that allows disparate topologies to cross over in a meaningful
way? (2) How can topological innovation that needs a few generations to be optimized
...
and Shavlik, 1997; Pujol and Poli, 1998; Yao and Liu, 1996; Zhang and M ¨uhlenbein,
...
method can use the evolution of topology to increase its efﬁciency.

2.1 TWEANN Encoding

The question of how to encode networks using an efﬁcient genetic representation must
...
M ¨uhlenbein, 1993). In contrast, indirect encodings usually only specify rules for con-
structing a phenotype (Gruau, 1993; Mandischer, 1993). These rules can be layer speciﬁ-
cations or growth rules through cell division. Indirect encoding allows a more compact
representation than direct encoding, because every connection and node are not speci-
ﬁed in the genome, although they can be derived from it.

2.1.1 Binary Encoding
Direct encodings usually require simpler implementations than indirect encodings.
The simplest implementation is based on the traditional bit string representation used
...
almost like a standard GA. However, there are several limitations as well. First, the size
...
TWEANNs use encodings that represent graph structures more explicitly. Pujol and
...
coming and outgoing connections. The idea is that different representations are appro-
...
CE is that its genetic representations are compact. Genes in CE can be reused multiple
times during the development of a network, each time requesting a cell division at a dif-
ferent location. CE shows that cell divisions can encode the development of networks
from a single cell, much as organisms in nature begin as a single cell that differentiates
as it splits into more cells.

Although CE demonstrates that it is possible to evolve developmental systems, we
chose direct encoding for NEAT because, as Braun and Weisbrod (1993) argue, indirect
...
we need to ﬁrst understand them well enough to make sure that they do not focus the
...
An even more difﬁcult form of competing conventions is present in TWEANNs,
...
peting conventions problem for ﬁxed or constrained topology networks such as nonre-
dundant genetic encoding (Thierens, 1996) do not apply. Radcliffe (1993) goes as far as
...
Genomes in nature are not of ﬁxed-length either. Somewhere along the evolution from
...
NEAT performs artiﬁcial synapsis based on historical markings, allowing it to add new
...
chance to optimize. It is unlikely that a new node or connection just happens to express
a useful function as soon as it is introduced. Some generations are required to optimize
the new structure and make use of it. Unfortunately, because of the initial loss of ﬁtness
...
hopes that in the future some useful connections will develop. However, nonfunctional
...
works with innovative structures could be isolated into their own species, they would
...
version of ﬁtness sharing introduced by Holland (1975) grouped individuals by perfor-
...
under many of the direct encoding schemes, there is a chance that a network will have
no path from each of its inputs to its outputs. Such infeasible networks take time to
weed out of the population.

However, there is a more subtle and more serious problem with starting randomly.
...
out with random topologies does not lead to ﬁnding minimal solutions, since the popu-
lation starts out with many unnecessary nodes and connections already present. None
of these nodes or connections have had to withstand a single evaluation, meaning there
is no justiﬁcation for their conﬁguration. Any minimization of networks would have to
be spent getting rid of apparatus that should not have been there in the ﬁrst place, and
...
One way to force minimal topologies is to incorporate network size into the ﬁtness
function, and some TWEANNs actually do this (Zhang and M ¨uhlenbein, 1993).
In
...
ﬁnding corresponding genes (as will be explained below).
...
That information is the historical origin of each gene. Two genes with the same histori-
cal origin must represent the same structure (although possibly with different weights),
since they are both derived from the same ancestral gene of some point in the past.
Thus, all a system needs to do to know which genes line up with which is to keep track
...
appearance of every gene in the system. As an example, let us say the two mutations in
Figure 3 occurred one after another in the system. The new connection gene created in
the ﬁrst mutation is assigned the number 7, and the two new connection genes added
...
whereas all excess or disjoint genes are always included from the more ﬁt parent. This
way, historical markings allow NEAT to perform crossover using linear genomes with-
...
this case, equal ﬁtnesses are assumed, so the disjoint and excess genes are also inherited
randomly. The disabled genes may become enabled again in future generations: there’s
a preset chance that an inherited gene is disabled if it is disabled in either parent.

out the need for expensive topological analysis.

By adding new genes to the population and sensibly mating genomes representing
...
Because smaller structures optimize faster than larger structures, and adding nodes and
...
novations they represent might be crucial towards solving the task in the long run. The
...
1−>41−>41−>41−>41−>42−>42−>42−>42−>42−>43−>4(cid:0)3−>4(cid:0)2−>52−>52−>52−>52−>55−>4(cid:1)5−>4(cid:1)5−>4(cid:1)5−>6(cid:1)5−>4(cid:1)5−>4(cid:1)1−>51−>56−>4(cid:2)6−>4(cid:2)1−>61−>61−>61−>55−>6(cid:1)5−>6(cid:1)3−>5(cid:0)3−>5(cid:0)3−>5(cid:0)6−>4(cid:2)3−>4(cid:0)3−>4(cid:0)3−>4(cid:0)DISABDISABDISAB(cid:3)DISAB(cid:3)DISABDISABDISABDISAB11111222223(cid:0)3(cid:0)444445(cid:1)5(cid:1)5(cid:1)6(cid:2)5(cid:1)5(cid:1)8(cid:4)8(cid:4)771010108(cid:4)6(cid:2)6(cid:2)9(cid:5)9(cid:5)9(cid:5)73(cid:0)3(cid:0)3(cid:0)disjoint(cid:6)disjoint(cid:6)disjoint(cid:6)excess(cid:7)excess(cid:7)Parent1(cid:8)Parent2(cid:8)Parent2Offspring(cid:9)Parent1(cid:8)1112(cid:10)2(cid:10)2(cid:10)3335(cid:11)5(cid:11)5(cid:11)6(cid:12)446(cid:12)4K. O. Stanley and R. Miikkulainen
...
c2D
N
The coefﬁcients c1, c2, and c3 allow us to adjust the importance of the three factors, and
...
population, which is crucial for speciated evolution to work. The adjusted ﬁtness f (cid:48)
i for
...
justed ﬁtnesses f (cid:48)
i of its member organisms. Species then reproduce by ﬁrst eliminating
...
3.4 Minimizing Dimensionality through Incremental Growth from Minimal
...
only those structures survive that are found to be useful through ﬁtness evaluations. In
...
advantage compared to other approaches, as will be discussed next.
...
The most difﬁcult problem of balancing two poles without velocity information, a non-
...
of 150 NEAT networks. The coefﬁcients for measuring compatibility were c1 = 1.0,
c2 = 1.0, and c3 = 0.4. With DPNV, c3 was increased to 3.0 in order to allow for ﬁner
...
to make room for the larger weight signiﬁcance coefﬁcient c3. If the maximum ﬁtness of
...
1+e−4.9x , at
logical diversity. We used a modiﬁed sigmoidal transfer function, ϕ(x) =
...
Because XOR is not linearly separable, a neural network requires hidden units to solve
it. The two inputs must be combined at some hidden unit, as opposed to only at the out-
...
for adding new nodes might be too destructive to allow new nodes to get into the pop-
...
Third, maybe the changing structure renders past connection weight values obsolete. If
so, the algorithm would have trouble enlarging topologies that are already largely spe-
cialized. This experiment is meant to show that NEAT is not impeded by such potential
...
so that higher ﬁtness would reﬂect better network structure. The resulting number was
squared to give proportionally more ﬁtness the closer a network was to a solution.
...
On 100 runs, the ﬁrst experiment shows that the NEAT system ﬁnds a structure for
...
sible network has a single hidden unit (Figure 5(b)). NEAT is very consistent in ﬁnding
...
TWEANN algorithms. Unfortunately, quantitative performance comparisons are dif-
...
In NEAT, a
to the entire initial population. Notice that there are no hidden nodes.
...
M ¨uhlenbein, 1993). Also, XOR is simple and artiﬁcial and does not challenge modern
...
and also because the difﬁculty can be adjusted. Earlier comparisons were done with a
single pole (Moriarty and Miikkulainen, 1996), but this version of the task has become
too easy for modern methods. Balancing two poles simultaneously is on the other hand
...
the neural network must apply force to the cart to keep the poles balanced for as long as
possible without going beyond the boundaries of the track. The system state is deﬁned
by the cart position x and velocity ˙x, the ﬁrst pole’s position θ1 and angular velocity ˙θ1,
...
Standard reinforcement learning methods have also been applied to this task (An-
...
from the neurons that are assembled into ﬁxed-topology networks for evaluation. The
...
and ESP. The success of CE was ﬁrst attributed to its ability to evolve structures. How-
...
though the difference between NEAT and ESP is not statistically signiﬁcant. The ﬁxed-
...
cally signiﬁcant (p < 0.001), except that between NEAT and ESP.
...
Evaluations Generations No. Nets
2048
100
200
200
...
that NEAT performs as well as ESP while ﬁnding more minimal solutions.

4.3.3 Double Pole Balancing Without Velocities
Gruau et al. (1996) introduced a special ﬁtness function for this problem to prevent the
system from solving the task simply by moving the cart back and forth quickly to keep
the poles wiggling in the air. (Such a solution would not require computing the missing
velocities.) Because both CE and ESP were evaluated using this special ﬁtness function,
NEAT uses it on this task as well. The ﬁtness penalizes oscillations. It is the sum of two
ﬁtness component functions, f1 and f2, such that F = 0.1f1 + 0.9f2. The two functions
...
for NEAT is 21,790 evaluations. Assuming similar variances for CE and ESP, all differ-
ences in number of evaluations are signiﬁcant (p < 0.001). The generalization results
...
giving each state variable (i.e., x, ˙x, θ1, and ˙θ1) each of the values 0.05, 0.25, 0.5, 0.75,
...
way in which structure is evolved has signiﬁcant impact on performance. NEAT is also
...
Why is NEAT so much faster than ESP on the more difﬁcult task when there was
...
Method
No-Growth NEAT (Fixed-Topologies)
Nonspeciated NEAT
Initial Random NEAT
Nonmating NEAT
Full NEAT

Evaluations Failure Rate
80%
25%
5%
0
...
ablation. The main result is that the system performs signiﬁcantly worse (p ≤ 0.001) for
...
from the system would disable NEAT by barring hidden nodes from all networks. NE
systems where structures are ﬁxed start with a fully connected hidden layer of neurons
...
identiﬁed among the different components of NEAT.
...
weight differences. Given 1,000 generations to ﬁnd a solution, the ablated system could
only ﬁnd a solution 20% of the time! When it did ﬁnd a solution, it took 8.5 times more
...
TWEANNs other than NEAT typically start with a random population (Angeline et al.,
...
The result is that random-starting NEAT was 7 times slower than full NEAT on average.
...
We have argued that speciation is important because it protects innovation and allows
search to proceed in many different spaces simultaneously. To test this claim, speciation
should be ablated from the system. However, if this is done and nothing else is changed
...
failed in 25% of the attempts. When it found a solution, it was 7 times slower on average
than full NEAT.
...
Starting minimallyGrowthSpeciationHistorical MarkingEvolving NN’s through Augmenting Topologies
...
For the last ablation, we removed mating from NEAT. This ablation tests the claim that
...
A total of 120 simulations were run with crossover disabled but all other settings
the same as before. It took on average 5,557 evaluations to ﬁnd a solution without mat-
ing, compared to 3,600 with mating enabled. The difference is statistically signiﬁcant
(p = 0.001). Thus, it is clear that mating does contribute when it is done right. However,
the nonmating version of NEAT is still signiﬁcantly faster than the other ablations.

5.7 Ablation Conclusions

An important conclusion is that all of the parts of NEAT work together (Figure 6). None
...
If speciation is
can no longer help NEAT ﬁnd spaces with minimal dimensionality.
...
without speciation, the system quickly converges onto a nonminimal topology that just
happens to be one of the best networks in the initial population. Thus, each component
is necessary to make NEAT work.

5.8 Visualizing Speciation

The ablation studies demonstrate that speciation is a necessary part of the overall sys-
tem. To understand how innovation takes place in NEAT, it is important to understand
...
We answer these questions by depicting speciation visually over time.
...
mean ﬁtness of the run, indicating that the species is a highly promising one. A species
...
A few species can be observed becoming extinct during this run. When a species
...
In the 21st generation, a structural mutation in the second-oldest surviving species
...
younger species also made its own useful connection, this time between the short pole
...
NEAT presents several advances in the evolution of neural networks. Through histor-
...
historical information in the population, such that costly topological analysis is not nec-
essary to speciate or mate networks. Our ablation studies conﬁrm the hypothesis that
starting with a population of minimal topologies is advantageous. Finally, performance
comparisons suggest that the evolution of structure can be used to gain efﬁciency over
the evolution of ﬁxed topologies.

A parallel can be drawn between structure evolution in NEAT and incremental
evolution (Gomez and Miikkulainen, 1997; Wieland, 1991). Incremental evolution is a
method used to train a system to solve harder tasks than it normally could by training it
on incrementally more challenging tasks. NE is likely to get stuck on a local optimum
...
evolving the solution to a harder task. The network structure before the addition is opti-
mized in a lower-dimensional space. When structure is added, the network increments
...
that adding structure is automatic in NEAT, whereas a sequence of progressively harder
tasks requires human design.

A key insight behind NEAT is that it is not the ultimate structure of the solution
that really matters, but rather the structure of all the intermediate solutions along the
way to ﬁnding the solution. The connectivity of every intermediate solution represents
...
pattern of weights on direct connections from the input nodes to the output nodes. The
connections from nodes one and two were always inhibitory, while those from nodes
three and four were always excitatory. NEAT also evolved a recurrent loop on the single
...
work can “kick” a ball towards the goal from any position on a ﬁeld, and another net-
work can dribble the ball around the ﬁeld without losing control. Neither of these two
...
optimum in ﬁtness space. By adding additional structure, NEAT adds new dimensions
to weight space, thereby opening up potential new avenues for escape. We plan to test
NEAT on problems with varying ﬁtness landscapes to get a better idea of the kinds of
...
The main conclusion is that NEAT is a powerful method for artiﬁcially evolving neural
...
Harvey, I. (1993). The Artiﬁcial Evolution of Adaptive Behavior Ph.D. thesis, School of Cognitive and
...
Applications to Biology, Control and Artiﬁcial Intelligence. University of Michigan Press, Ann
Arbor, Michigan.

Kaelbling, L. P., Littman, M., and Moore, A. W. (1996). Reinforcement learning: A survey. Journal

of Artiﬁcial Intelligence, 4:237–285.
...
Reeves, C. R., and Steele, U. C., editors, Artiﬁcial Neural Nets and Genetic Algorithms, pages
...
Artiﬁcial Intelligence, pages 762–767, Morgan Kaufmann, San Francisco, California.
...
Moriarty, D. E. and Miikkulainen, R. (1997). Forming neural networks through efﬁcient and adap-
...
domains. Technical Report UNSW-CSE-TR-9410, School of Computer Science and Engineer-
...
Zhang, B.-T. and M ¨uhlenbein, H. (1993). Evolving optimal neural networks using genetic algo-
